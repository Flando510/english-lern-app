...
authForm.addEventListener("submit", async function(event) {
  ...
  if (isRegisterMode) {
    authButton.textContent = "Konto wird erstellt...";
  } else {
    authButton.textContent = "Anmeldung...";
  }
  ...
  try {
    ...
    if (isRegisterMode) {
      ...
      if (profileInsert.error) {
        console.error(profileInsert.error);
      ...
      await showApp(signup.data.user);
...
async function addPoints(amount) {
  ...
  currentProfile.points = result.data.points;
    ...
    words.map(function(word) {
      ...
      return (
        '<div class="vocab-card">' +
          "<strong>" + escapeHtml(word[0]) + "</strong>" +
          "<span>" + escapeHtml(word[1]) + "</span>" +
        "</div>"
      );
