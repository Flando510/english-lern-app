...
async function renderQuiz() {
  ...
  // Wenn noch keine Quiz-Auswahl getroffen wurde
  if (!quizSource || quizQuestions.length === 0) {

    learningArea.innerHTML = `
      <h2>🧠 Quiz – ${currentClass}. Klasse</h2>

      <p>Wähle aus, was du üben möchtest:</p>

      <div class="content-grid">

        <button class="quiz-source-btn" data-source="all">
          📚 Alle Vokabeln
        </button>

        <button class="quiz-source-btn" data-source="unit1">
          📘 Unit 1
        </button>

        <button class="quiz-source-btn" data-source="unit2">
          📗 Unit 2
        </button>

        <button class="quiz-source-btn" data-source="unit3">
          📕 Unit 3
        </button>

        <button class="quiz-source-btn" data-source="unit4">
          📙 Unit 4
        </button>

        <button class="quiz-source-btn" data-source="grammar">
          📖 Grammatik
        </button>
    ...
    document
      ...
      .forEach(button => {
        ...
        button.addEventListener("click", async () => {
          ...
          quizSource = button.dataset.source;
          currentQuestion = 0;
  ...
  learningArea.innerHTML = `
    ...
    <button id="quizBackButton" class="secondary-btn">
      ← Auswahl ändern
    </button>
...
async function loadQuizQuestions() {
  ...
  if (quizSource.startsWith("unit")) {
    ...
    const unitNumber =
      Number(quizSource.replace("unit", ""));

    query =
      query.eq("unit_number", unitNumber);
  }
  ...
  if (!data || data.length < 3) {
    ...
    learningArea.innerHTML = `
      ...
      <p style="color:#c0392b;">
        Für diesen Bereich sind noch nicht genug
        Vokabeln vorhanden.
      </p>
  ...
  quizQuestions =
    shuffledWords.map(word => {
      ...
      return {
        ...
        options,
        answer: options.indexOf(word.german)
      };
    });
...
function renderExercise() {
  ...
  learningArea.innerHTML = `
    ...
    <div class="example">
      ...
      Verwende das Wort
      <strong>${lessons[currentClass].vocab[0][0]}</strong>
