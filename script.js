...
async function saveAdminVocab() {
   ...
   * german
   *
   * Der Beispielsatz wird nur gespeichert,
   * wenn deine Tabelle dafür eine Spalte
   * besitzt.
   */
  ...
  /*
   * Wenn deine vocab-Tabelle
   * eine example-Spalte hat,
   * wird sie mitgespeichert.
   */
...
/*
 * Die HTML-Datei benutzt beim Button
 * type="submit".
 *
 * Deshalb zusätzlich:
 */
...
if (addVocabButton) {
  ...
  addVocabButton.addEventListener(
    ...
    async event => {
      ...
      /*
       * Nur ausführen,
       * falls das Formular nicht
       * automatisch submitted wird.
       */

      if (
        adminVocabForm &&
        event.target === addVocabButton
      ) {
